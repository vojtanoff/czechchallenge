...
# https://vojtanoff.github.io/czechchallenge-draft/
#
# Od 17. 9. 2026 je ve VLASTNÍM repu. Po přepnutí czechchallenge.com na Pages by
# pracovní verze i editor katetru byly veřejně na oficiální doméně kongresu.
# Obrázky se tahají z ostrého webu, aby se 23 MB fotek nemuselo duplikovat.
...
set -e
Z=~/dev/czechchallenge-pexa-final; C=~/dev/czechchallenge-draft
mkdir -p "$C"
for f in index.html gallery.html style.css style-pexa.css efekty.js content.json katetr.json editor-katetr.html; do
  [ -f "$Z/$f" ] || continue
  python3 - "$Z/$f" "$C/$f" <<'PY'
import sys
OBR="https://vojtanoff.github.io/czechchallenge/img/"   # obrázky z ostrého webu
src,dst=sys.argv[1],sys.argv[2]; s=open(src,encoding="utf-8").read()
if dst.endswith(".html"):
    s=s.replace('src="img/','src="'+OBR).replace('href="img/','href="'+OBR)
    s=s.replace('fetch("content.json",{cache:"no-store"}).then(function(r){ return r.json(); }).then(function(C){',
                'fetch("content.json",{cache:"no-store"}).then(function(r){ return r.json(); }).then(function(C){ C=JSON.parse(JSON.stringify(C).replace(/"img\\//g,\'"'+OBR+'\'));')
    ...
    s=s.replace('url(img/','url('+OBR).replace('url("img/','url("'+OBR)
open(dst,"w",encoding="utf-8").write(s)
PY
done
rsync -a --delete "$Z/fonts/" "$C/fonts/"
echo "czechchallenge-draft aktualizováno z $(cd $Z && git log --oneline -1) – nezapomeň commit a push v ~/dev/czechchallenge-draft"
